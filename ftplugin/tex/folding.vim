" copy from https://tex.stackexchange.com/questions/130586/vim-nice-folding-function
" which based on latex-box plugin
" Initialization {{{
...
function! LatexBox_FoldText()
    ...
    if line =~ '\\begin'
        ...
        elseif env == 'table'
            ...
            let title = 'Table: ' . caption
            " todo: for title of table env, if label -> use label,
            " elif caption -> use caption, else -> 
        elseif env == 'figure'
            let label = s:LabelEnv()
            let caption = s:CaptionEnv()
            let title = 'Figure: ' . caption
            " todo: for title of figure env: use filename + label
        elseif env == 'frontmatter'
            let title = 'Frontmatter'
        else
            let label = s:LabelEnv()
            let caption = s:CaptionEnv()
